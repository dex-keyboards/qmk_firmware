...
void oled_task_motion(uint32_t time, uint32_t delta, vec16_t offset){

    oled_write_sprite_string_positioned("motion", 6, add_vec16((vec16_t){32, 8}, offset), -1);

    const char* motion_str = get_u16_str(get_motion_cpi_step() * CPI_STEP, ' ');

    oled_write_sprite_string_positioned((char*)motion_str, 5, add_vec16((vec16_t){32, 40}, offset), 10);
}
